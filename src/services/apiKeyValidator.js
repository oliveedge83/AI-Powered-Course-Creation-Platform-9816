import axios from 'axios';
          ...
          switch (status) {
            ...
            case 429:
              return {
                isValid: false,
                error:
                  'Rate limit exceeded. Your API key is valid but you have exceeded the rate limit.',
    ...
    export const validateApiKeys=async (apiKeys)=> {
      if (!apiKeys || apiKeys.length===0) {
        return {hasValidKey: false,error: 'No API keys provided',validKey: null};
      }
      ...
      for (let i=0;i < apiKeys.length;i++) {
        ...
        console.log(
          `Validating API key ${i + 1}/${apiKeys.length}: ${
            keyData.label || 'Unlabeled'
          }`
        );
        try {
          ...
          if (validation.isValid) {
            ...
            console.log(
              `❌ Invalid API key: ${keyData.label || 'Unlabeled'} - ${
                validation.error
              }`
            );
            ...
            if (validation.errorType==='RATE_LIMIT_ERROR') {
              console.log(
                `⚠️ API key is valid but rate limited: ${keyData.label || 'Unlabeled'}`
              );
              return {
                hasValidKey: true,
                validKey: keyData,
    ...
    export const quickValidateKeyFormat=(apiKey)=> {
      if (!apiKey || typeof apiKey !=='string') {
        return {isValidFormat: false,error: 'API key is required and must be a string'};
      }
      if (!apiKey.startsWith('sk-')) {
        return {isValidFormat: false,error: 'API key must start with "sk-"'};
      }
      if (apiKey.length < 40) {
        return {isValidFormat: false,error: 'API key appears to be too short'};
      }
      return {isValidFormat: true,message: 'API key format is correct'};
    };
    
    /**
     * Fetches and filters available OpenAI models
     * @param {string} apiKey - The OpenAI API key
     * @returns {Promise<Array>} - A sorted and filtered list of model IDs
     */
    export const fetchOpenAIModels=async (apiKey)=> {
      try {
        ...
        if (response.data && Array.isArray(response.data.data)) {
          ...
          const requiredTimestamp=1744310000;
          const filteredModels=models
            .filter(
              (model)=>
                model.created >=requiredTimestamp &&
                model.id.includes('gpt') &&
                !model.id.includes('vision') &&
                !model.id.includes('instruct')
            )
            .map((model)=> ({id: model.id,created: model.created}));
    
          // Sort by creation date descending,with special models at the top
          filteredModels.sort((a,b)=> {
            const specialModels=['gpt-5-mini-2025-08-07','gpt-5-mini','gpt-5'];
            const aIsSpecial=specialModels.indexOf(a.id);
            const bIsSpecial=specialModels.indexOf(b.id);
    
            if (aIsSpecial > -1 && bIsSpecial > -1) {
              return aIsSpecial - bIsSpecial;// Sort by order in specialModels
            }
          ...
          console.log(`✅ Fetched and filtered ${filteredModels.length} models.`);
          return filteredModels.map((m)=> m.id);
        }
        return [];
      } catch (error) {
        console.error('Error fetching OpenAI models:',error);
        // Don't throw,return a default list as a fallback
        return ['gpt-5-mini-2025-08-07','gpt-5-mini','gpt-5'];
      }
    };