...
export const GenerationProvider = ({ children }) => {
  ...
  const startGeneration = (courseTitle, totalTopics, totalLessons, initialMessage = 'Starting generation process...') => {
    ...
    setGenerationStatus({
      ...
      totalTasks: totalLessons * 3, // Each lesson has 3 tasks: content, slides, voice-over
      completedTasks: 0,
      estimatedTimeRemaining: null,
      startTime: Date.now(),
      details: {
  ...
  const updateTaskProgress = (completedTasks, currentTask, details = {}) => {
    const progress = (completedTasks / generationStatus.totalTasks) * 100;
