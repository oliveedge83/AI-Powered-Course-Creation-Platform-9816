import axios from 'axios';
    ...
    import {validateAITableCredentials,postCourseToAITable,generateLessonSlidesJson,generateTopicLessonStructure,generateCourseUniqueId,} from './aitableService';
    ...
    const generateWebSearchContext=async (
      ...
      try {
        ...
        const buildPerplexityOptions=(maxTokens)=> {
          ...
          if (config.sonarModel.includes('sonar')) {
            ...
            if (
              ...
              options.web_search_options.search_recency_filter=
                config.searchRecency;
            }
        ...
        console.log(
          `🔧 Using search_mode: ${
            config.searchMode==='web_search' ? 'web' : config.searchMode
          }`
        );
        ...
        console.log(
          `🎉 Web search context generation completed for topic: ${topic.topicTitle}`
        );
        console.log(
    ...
    const generateReadingContentWithTwoStageRAG=async (
      ...
      abortSignal=null,
      model='gpt-5-mini-2025-08-07',// ✅ NEW: Add model parameter
      gpt5Params={}
    )=> {
      try {
        console.log(
        ...
        console.log(
          `👥 Using audience context: ${
            audienceContext || 'Default (Procure to pay professionals)'
          }`
        );
    ...
    Reading Content (first 2000 chars): ${plainTextContent.substring(
      0,
      2000
    )}...
    ...
    Remember to return ONLY the valid JSON object.`;
        ...
        if (jsonStart===-1 || jsonEnd===-1 || jsonEnd <=jsonStart) {
          console.error(
            '❌ No JSON found in response for additional sections:',
            cleanContent
          );
          throw new Error(
            'No valid JSON object found in AI response for additional sections.'
          );
        }
    ...
    const callOpenAI=async (
      ...
      for (let attempt=1;attempt <=retries + 1;attempt++) {
        try {
          console.log(
            `API call attempt ${attempt}/${retries + 1} using model: ${modelName}`
          );
    ...
    export const generateCourseContent=async (
      ...
      try {
        ...
        const audienceContext=extractAudienceFromContext(
          programContext.programContext
        );
    
        // ✅ NEW: Create LMS service using factory pattern
        console.log(
          `🏭 Creating ${lmsCredentials.lmsType || 'tutor'} LMS service...`
        );
        ...
        if (recoveryState?.lmsData) {
          ...
          for (let attempt=1;attempt <=3;attempt++) {
            try {
              const courseResponse=await lmsService.createCourse(
                course,
                abortSignal
              );
              ...
              console.log(
                `Retrying LMS course creation in ${retryDelay / 1000} seconds...`
              );
              await new Promise((resolve)=> setTimeout(resolve,retryDelay));
            }
          }
        ...
        for (let i=currentTopicIndex;i < (course.topics?.length || 0);i++) {
          ...
          if (recoveryState?.topics?.[i]?.topicId) {
            ...
            try {
              ...
              console.log(
                `Created topic with ID: ${topicId} using ${lmsService.getLMSType()}`
              );
          ...
          if (usePerplexityWebSearch && perplexityApiKey) {
            ...
            try {
              console.log(
                `🌐 Starting web search context generation for topic: ${topic.topicTitle}`
              );
              // Log the parameters
              ...
              console.log(
                ` - Topic Context Length: ${topicWebSearchContext.length} chars`
              );
          ...
          for (let j=startLessonIndex;j < (topic.lessons?.length || 0);j++) {
            ...
            onProgress?.(
              ...
              `Generating content for lesson ${j + 1} of ${
            ...
            try {
              ...
              const readingResult=await generateReadingContentWithTwoStageRAG(
                ...
                abortSignal,
                model,// ✅ NEW: Pass the selected model
                gpt5Params
              );
              ...
              if (readingTokenUsage) {
                courseTokenUsage.prompt_tokens +=
                  readingTokenUsage.prompt_tokens || 0;
                courseTokenUsage.completion_tokens +=
                  readingTokenUsage.completion_tokens || 0;
                courseTokenUsage.total_tokens +=
                  readingTokenUsage.total_tokens || 0;
              }
              ...
              // Only one task now for all additional sections
              ...
              console.error(
                `Error generating content for lesson ${lesson.lessonTitle}:`,
                error
              );
              await supabase.from('api_logs_74621').insert([
                {
          ...
          if (topicLessonContents.length > 0) {
            ...
            try {
              ...
              if (questionsData && questionsData.tokenUsage) {
                ...
                courseTokenUsage.total_tokens +=
                  questionsData.tokenUsage.total_tokens || 0;
              }
              completedTasks++;
            } catch (quizError) {
        ...
        if (aitableValidated && aitableConfig) {
          ...
          try {
            ...
            if (aitableResult.success) {
              ...
              console.warn(
                '⚠️ Failed to post course data to AITable:',
                aitableResult.error
              );
              await supabase.from('api_logs_74621').insert([
                {
        ...
        try {
          ...
          console.error(
            'Failed to update course token usage in Supabase:',
            updateError
          );
        }
    ...
    export const generateCourseTopicsAndLessons=async (
      ...
      try {
        console.log(
          `Generating detailed topics and lessons for course: ${course.courseTitle}`
        );
