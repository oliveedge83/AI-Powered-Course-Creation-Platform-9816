import axios from 'axios';
    ...
    export const generateCourseContext=async (
      programData,
      validationResults,
      ...
      try {
        ...
        console.log(
          `Using ${bestApiKey.provider} API for content generation: ${bestApiKey.label}`
        );
        ...
        const researchPrompt=`
          ...
          ${
            industryResearch
              ? `CURRENT INDUSTRY RESEARCH AND TRENDS: ${industryResearch}
          ...
          {"programTitle": "A prestigious and descriptive title for the MicroMasters program.","programLevelOutcomes": "A numbered list (as a single string,use '\\n' for new lines) of 4-6 high-level outcomes a student will achieve.","targetAudience": "A detailed description of the ideal profile for this program.","industryRelevance": "A brief analysis of why this program is currently relevant,citing industry trends and PESTEL analysis of key industry drivers. (as a single string with numbered points)","hardHittingPainPoints": "A numbered list (as a single string,use '\\n' for new lines) of the most significant anxieties,frustrations,or roadblocks for practitioners in this niche.","keyEmotionalTriggers": "A numbered list (as a single string,use '\\n' for new lines) of the deep-seated emotional drivers for actionable results in this field.","mostImpactfulOutcomes": "A numbered list (as a single string,use '\\n' for new lines) of the most powerful results and business outcomes practitioners are aiming at.","foundationalKnowledge": "A description of the prerequisite knowledge or experience a student should have.","instructionalDesignFramework": "Recommend a framework suitable for a multi-course,advanced program."}
        `;
              ...
              messages: [
                {
                  role: "system",
                  content:
                    "You are a senior academic strategist and career analyst with deep expertise in curriculum design and industry analysis. You excel at integrating current market research and trends into educational program design. YOU MUST ONLY OUTPUT A VALID JSON OBJECT.",
                },
    ...
    export const generateProgramStructure=async (
      programData,
      validationResults,
      usePerplexityResearch=true
    )=> {
      try {
        ...
        if (programData.useSonarProStructure) {
          ...
          console.log(
            `✅ Using Perplexity Sonar-Pro for structure generation: ${structureApiKey.label}`
          );
        } else {
          ...
          console.log(
            `✅ Using ${structureApiKey.provider} API for program generation: ${structureApiKey.label}`
          );
        }
    
        // Step 1: Generate comprehensive course context with enhanced research
        console.log("📊 Generating course context...");
        const courseContextResult=await generateCourseContext(
          programData,
          validationResults,
        ...
        const structurePrompt=`
          ...
          ${programData.suggestedCourseStructure ? `- User Suggested Course Structure (use as a strong guideline): ${programData.suggestedCourseStructure}` : ''}
          ...
          {"programContext": "The full program context JSON string here","summaryProgramContext": "A concise 2-3 sentence summary of the program context for use in subsequent content generation","researchEnhanced": ${usePerplexityResearch},"usedSonarProStructure": ${programData.useSonarProStructure || false},"courses": [
            {
              "id": "course-1",
              ...
              messages: [
                {
                  role: "system",
                  content:
                    "You are an expert educational content designer specializing in professional MicroMasters programs. Create comprehensive,industry-aligned curriculum structures with detailed content that reflects current market demands and trends. ALWAYS respond with ONLY a valid JSON object - no explanatory text.",
                },
        ...
        const numberedProgramData=applyNumberingToProgram({
          ...programData,
          ...parsedData,
        });
    
        console.log(
          `🎉 Enhanced program structure generated successfully using ${programData.useSonarProStructure ? 'Sonar-Pro' : structureApiKey.provider} with numbering system`
        );
        console.log(
          `📊 Generated ${numberedProgramData.courses.length} courses with ${numberedProgramData.courses.reduce((total,course)=> total + course.topics.length,0)} total topics`
        );
    ...
    export const generateCourseTopicsAndLessons=async (
      ...
      try {
        console.log(
          `Generating enhanced topics and lessons for course: ${course.courseTitle}`
        );
              ...
              messages: [
                {
                  role: "system",
                  content:
                    "You are an expert instructional designer specializing in professional education. Create detailed,practical course content with comprehensive lesson descriptions. Each lesson must be unique,specific,and tailored to the course context with current industry relevance.",
                },
